...
#if !defined(_DISABLE_ALBEDO_MAP) || defined(_CHANNEL_MAP) || defined(_DISTANCE_TO_EDGE)
            #define _UV
#else
            #undef _UV
#endif
...
#define PROXIMITY_LIGHT_DATA_SIZE 6
            ...
            fixed RoughAverageOfThree(fixed3 averager)
            {
                return (averager.x + averager.y + averager.z) * 0.333f;
            }
            ...
            fixed4 frag(v2f i, fixed facing : VFACE) : SV_Target
            {
                ...
                clip(max(albedo.a, pointToLight * _FluentLightIntensity) - _Cutoff); // Crucial for the AlphaTest(osterone).
                albedo.a = 1.0;
